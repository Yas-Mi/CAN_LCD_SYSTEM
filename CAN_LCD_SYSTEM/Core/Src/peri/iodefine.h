...
#include "stm32f446xx.h"	// これがiodefineなるもの
...
// フィールド値取得
#define get_field(reg,bno)				(((reg)&(msk))>>get_sft(msk))
#define get_field_t(type,reg,bno)		((type)(((reg)&(msk))>>get_sft(msk)))

// フィールド値設定
#define set_field(reg,bno,val)			((reg)=(((reg)&(~(msk)))|((val)<<get_sft(msk))))
#define set_field_t(type,reg,bno,val)	((reg)=(type)((type)((reg)&(~(msk)))|(type)((val)<<get_sft(msk))))
